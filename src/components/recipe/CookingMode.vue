<template>
  <div class="fixed inset-0 z-[9999] bg-slate-950 flex flex-col">
    <div class="flex items-center justify-between px-8 py-5 border-b border-slate-800">
      ...
      <button @click="$emit('close')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white transition-all">
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
      </button>
    </div>

    <div class="flex flex-1 overflow-hidden">
      <div class="w-72 flex-shrink-0 border-r border-slate-800 overflow-y-auto p-6">
        <h3 class="font-display text-lg font-semibold mb-4 text-slate-300">Ingrediënten</h3>
        <div class="space-y-2">
          <div v-for="ing in scaledIngredients" :key="ing.id" class="flex gap-2 text-sm py-2 border-b border-slate-800/50">
            <span class="text-saffron-400 font-medium w-24 flex-shrink-0">{{ ing.scaledQty }} {{ ing.unit }}</span>
      ...
      <div class="flex-1 flex flex-col items-center justify-center p-12">
        ...
        <div class="text-center max-w-2xl">
          <div class="text-8xl font-display font-bold text-saffron-500/20 mb-2">{{ stepIdx + 1 }}</div>
          <p class="text-2xl text-slate-100 leading-relaxed font-medium">{{ currentStep?.text }}</p>
        </div>

        <div class="flex items-center gap-6 mt-16">
          <button @click="prev" :disabled="stepIdx === 0"
            :class="['p-4 rounded-2xl border transition-all', stepIdx === 0 ? 'border-slate-800 text-slate-700 cursor-not-allowed' : 'border-slate-600 text-slate-300 hover:border-saffron-500 hover:text-saffron-500']">
          ...
          <button @click="next" :disabled="stepIdx === total - 1"
            :class="['p-4 rounded-2xl border transition-all', stepIdx === total - 1 ? 'border-slate-800 text-slate-700 cursor-not-allowed' : 'border-saffron-500 bg-saffron-500/10 text-saffron-500 hover:bg-saffron-500/20']">
